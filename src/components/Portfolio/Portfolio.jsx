      ...
      <section className={wrap_container}>
        ...
        <div className={dashboard} role="main">
          <div className={dashboard__sidebar}>
            <h1 className={vcard}>
              <picture>
                <img className="photo" alt="User" src={imageUrl} />
              </picture>
            ...
            <div className={classes["profile-actions"]}>
              <div className={classes["profile-message"]}>
                <a
                  className={contact + " " + classes["btn-text"]}
                  rel="tipsy"
              ...
              <div className={classes["profile-follow-prompt"]}>
                <div className={classes["btn-text"]} onClick={followHandler}>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    enableBackground="new 0 0 24 24"
                    viewBox="0 0 24 24"
            ...
            <div className={classes["profile-extra"]}>
              <ul>
                <li>
                  <svg className={classes["profile-extra__icon"]}>
                    <use xlinkHref={`/assets/img/sprite.svg#icon-earth`} />
                  </svg>
                  <a
                    href={website}
                    target="_blank"
          ...
          <div className={dashboard__notification}>
            {!userHandle && (
              <div className={classes["dashboard__notification-action"]}>
                <Link className={classes["action__link"]} to={"/edit-profile"}>
                  {
                    <EditIcon
                      userHandle={userHandle}
                      icon_label={null}
                ...
                <Link className={classes["action__link"]} to={"/add-education"}>
                  {
                    <EditIcon
                      userHandle={userHandle}
                      icon_label={null}
            ...
            <div className={skill1}>
              <h2 className={classes["section-subtitle"]}>SKILLS</h2>
              <ul className={classes["skills-list"]}>
                {info_3_0_a && info_3_0_a.split(",").length > 0 ? (
                  info_3_0_a.split(",").map((skill, i) => (
                    <li key={i}>
                      <p className={skill__item}>{skill}</p>
                    </li>
                  ))
                ) : (
                  <li>
                    <p className={skill__item}>No skills added</p>
                  </li>
                )}
              </ul>
            </div>
            {!userHandle && (
              <div className={notification}>
                ...
                <p className={notification__msg}>No notification.</p>
              </div>
            )}
          </div>
        </div>
            ...
            {experience.length > 0 ? (
              experience.map((exp, id) => (
                <div key={id} className={_container}>
                  ...
                  <p className={date}>{exp.from + "-" + exp.to}</p>
                  <p className={description}>{exp.description}</p>
                </div>
              ))
            ) : (
              <div className={_container}>No Experience is been added.</div>
            )}
          </div>
        </div>
      </section>
